...
function getCurrentDateContext(): {
  ...
  return {
    ...
    newYearInstruction: `お正月の画像では、今年の干支である${zodiac.reading}を、自然な動物または無地の置物として必ず1つ取り入れてください。別の年の干支は入れないでください。文字生成の誤りを防ぐため、画像内には漢字・ひらがな・カタカナ・中国語・英字・数字・年号・日付・記号・ロゴなど、読める文字を一切描かないでください。この指示は年号や干支の文字を正確に描く指示より優先し、正しい文字であっても表示禁止です。絵馬・札・看板・年賀状・掛け軸・衣装・首飾り・胸当て・メダル・置物・干支の体や台座も、文字なしの無地または日本の伝統文様だけにしてください。干支の動物には文字入りの赤金の胸当て・前掛け・鞍・札・メダルを付けないでください。全体は日本のお正月らしい落ち着いた和風に限定し、畳・障子・自然な木・神社の鳥居・門松・鏡餅・しめ縄・紅白の水引・和紙・日本の伝統文様を用いてください。配色は紅白・松の緑・木の色を基調とし、金色は控えめな差し色だけにしてください。中国風の寺院建築、春節の赤提灯、丸い吊り飾り、房の付いた中国結び、赤と金を過剰に使った装飾、中国の硬貨・赤い封筒・龍舞・獅子舞など、中国の春節に見える要素は一切入れないでください。`,
    zodiacMainInstruction: `この画像は、お正月の3枚のうち干支が主役の1枚です。${zodiac.reading}の動物または文字のない素朴な置物を画面で最も目立つ主役にし、日本の正月飾りと調和する構図にしてください。アップロードされた犬本人も、干支の主役を引き立てる位置に自然に一緒に写してください。犬を${zodiac.reading}に置き換えたり、犬の顔・毛並み・体型を変えたりすることは絶対に禁止です。`,
  };
}
...
const EVENTS: Record<string, string[]> = {
  ...
  halloween: [
    ...
    "ドラキュラのコスチュームを着て、トリックオアトリートのバケツを前足の横に置き、ハロウィンで飾られた玄関先に自然に立っている可愛い画像を生成してください。",
  ],
  christmas: [
