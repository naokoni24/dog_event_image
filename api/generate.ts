...
// 全犬種共通の維持指示。コスチュームやポーズの演出は許可（通常の挙動）。
const FACE = "この犬の顔・顔の形・目の形と間隔・鼻の形と大きさ・口元・マズルの長さと丸み・耳の形・毛の質感と色は、元の写真と同一の個体に保ってください。別の犬の顔や、一般的なトイプードル顔・テディベア風の別顔に変えることは絶対に禁止。顔立ちを美化・若返り・補正しないでください。表情のみ少し変えることは許可します。写真に複数の犬が写っている場合は、それぞれを個別の個体として認識し、各犬の顔・毛色・毛並みを元の写真のまま維持してください。";
...
const KEEPS = [
  ...
  // 2枚目：通常
  `この犬の犬種・毛並み・毛色・年齢感・顔立ちを完全にそのまま維持してください。${FACE}元の写真で犬が着ている服や洋服は生成画像では着せないでください。後ろ向きは禁止。全体的にかわいい雰囲気で仕上げてください。`,
  // 3枚目：少し舌出し（茶色トイプードル時は POODLE_LOCK が優先され表情も固定される）
...
// 茶色トイプードル“限定”の追加ロック。別顔化が起きやすいため、この犬種・毛色の
// ときだけ犬本体を固定して変形を最小化する（それ以外の犬では無視される）。
const POODLE_LOCK = " 【条件付き・茶色トイプードル限定】もし写真の犬が茶色〜アプリコット系のトイプードルである場合に限り、その犬本体（顔・表情・ポーズ・体の向き・大きさ・写真内の位置・トリミング）を元写真のピクセルのまま固定し、犬を描き直さないでください。この場合、以下のシーン指示のうち犬のポーズ・動作・着用衣装に関する部分は無視し、背景・周囲の小物・頭に軽く乗せる程度の装着物だけを採用してください。テディベア風の別顔や整ったプードル顔に寄せること、毛色の濃淡・巻き毛の密度・目鼻の位置を変えることは厳禁です。茶色トイプードル以外の犬種・毛色では、この段落全体を無視して通常どおり仕上げてください。";
...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  const prompt = `${KEEPS[promptIndex]}${POODLE_LOCK}${prompts[promptIndex]}${STYLE}`;
